...
import { ACTIONS, DECISIONS, ROLES, type MyContext } from './interfaces/bot-context.interface';
import { PrismaService } from 'src/prisma/prisma.service';
import { CallbackQuery } from 'telegraf/types';
import { extractFileId } from './helpers';
import { adminCommands, EMPTY_STRING, MAX_SPECTATORS_COUNT, MESSAGES, userCommands, ZERO } from 'src/constants';
...
export class BotService {
  ...
  async handleText(ctx: MyContext) {
    ...
    const user = await this.prisma.user.findUnique({ where: { telegramId: String(ctx.from!.id) } });
    ...
    if (user.role === ROLES.ADMIN) {
      switch (data) {
        case ACTIONS.ANNOUNCEMENT:
          ...
          const users = await this.prisma.user.findMany({ where: { role: ROLES.USER } });
          await Promise.all(
            users.map(u =>
              ctx.telegram.sendMessage(u.telegramId, MESSAGES.NEW_RECORDING_NOTE)
            )
          );
        ...
        case ACTIONS.LIST_VIEWERS:
          ...
          const list = allSpectators.length
            ? allSpectators.map((s, i) => `${i + 1}. ${s.user.fullName} — ${s.user.passport}`).join('\n')
        ...
        case ACTIONS.LIST_ATTENDEES:
          ...
          const message = Object.values(attendees)
            .map((g, i) => {
              return `${i + 1}👤 ${g.user.fullName} - ${g.user.passport}\n`;
            })
            .join('\n\n');
        ...
        case ACTIONS.SEND_NOTIFICATION:
          ...
          const spectators_list = await this.prisma.spectators.findMany({ include: { user: true } })

          await Promise.all(
            [...spectators_list].map(u =>
              ctx.telegram.sendMessage(u.user.telegramId, MESSAGES.NOTIFICATION)
            )
          );

          await Promise.all(
            Object.values(attendees_list).map(user => {
              ctx.telegram.sendMessage(user.user.telegramId, MESSAGES.NOTIFICATION)
            })
          );
  ...
  private async startSorting(ctx: MyContext) {
    ...
    const grouped = pendingWorks.reduce((acc, w) => {
  ...
  private async showCurrentGroup(ctx: MyContext) {
    ...
    await ctx.reply(`👤 User: ${user.fullName}\n📌 Ishlar soni: ${group.length}`);

    for (const work of group) {
      if (work.fileId) {
        await ctx.telegram.sendDocument(chatId, work.fileId, { caption: work.text ?? EMPTY_STRING });
      } else if (work.text) {
  ...
  private async handleSortingDecision(ctx: MyContext, action: string) {
    ...
    await ctx.reply(
      action === DECISIONS.ACCEPTED ? MESSAGES.ACCEPT_NOTE : MESSAGES.REJECT_NOTE
    );

    await ctx.telegram.sendMessage(
      userTelegramId,
      action === DECISIONS.ACCEPTED ? MESSAGES.USER_ACCEPT_NOTE : MESSAGES.USER_REJECT_NOTE
    );
  ...
  private async getAcceptedUsers() {
    ...
    const groupedByUser = acceptedWorks.reduce((acc, work) => {
